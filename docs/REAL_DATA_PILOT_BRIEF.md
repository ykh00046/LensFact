...
이 절은 위 원문을 고쳐 쓰지 않고 덧붙인다. 원문은 2026-08-27 시점의 기록이며, 아래 항목이 그 이후의 결정으로 원문을 대체한다.
근거는 `docs/verification/audit-pilot-2026-08-28/FINDINGS.md`와 같은 폴더의 `EVIDENCE.md`, 적용 기록은 `docs/verification/CORRECTIONS.md`에 있다.

### 수용 기준 5번 개정

원문 5번은 허가번호 3개를 `수허 16-499 호`, `수허 13-112 호`, `수허 08-131`로 표시하라고 적었다.
2026-08-28 감사에서 MFDS 의료기기 UDI 원장을 전수 대조한 결과, 원장 표기는 세 제품 모두 `호`가 붙고 `수허`와 숫자 사이에 공백이 있는 형태였다.
따라서 5번을 다음과 같이 개정한다.

> 5. 허가번호 3개의 **화면 표시값은 MFDS 원장 표기를 따른다**: `수허 16-499 호`, `수허 13-112 호`, `수허 08-131 호`.
>    각 제조사 문서의 표기는 바꾸지 않고 해당 출처 레코드의 `raw`에 인쇄된 그대로 보존한다.

- `수허 16-499 호` — 원장 497건 전수 집계, distinct 신원 1건. 한국 IFU 원문은 공백이 없는 `[수입허가번호] 수허16-499 호`이며 그대로 두 번째 출처의 `raw`에 남긴다.
- `수허 13-112 호` — 원장 105건. 2026-08-28 재실행에서 2026-08-27 캡처와 완전히 일치했다. 표시값 변경 없음.
- `수허 08-131 호` — 원장 94건(쿠퍼비전코리아(주) · 연속착용 소프트 콘택트렌즈 · 등급 3 · Biofinity 64건 / Biofinity XR 30건). **표시값이 `수허 08-131`에서 `수허 08-131 호`로 바뀌었다.** 한국 전체 제품 목록의 각주 원문 `수허 08-131 바이오피니티 & 바이오피니티 XR`은 `호`가 빠진 채 두 번째 출처의 `raw`에 그대로 남긴다.

번호는 세 건 모두 같고 표기만 달랐다. 표시값을 원장 표기로 통일한 이유는, 독자가 화면의 문자열을 그대로 MFDS 조회창에 넣었을 때 검색이 재현되어야 하기 때문이다.

### 제품 3 법인명 표기

원문 제품 3의 `쿠퍼비젼코리아(주)`는 오기다. MFDS 원장과 쿠퍼비전코리아 한국 공식 페이지 모두 `쿠퍼비전코리아(주)`이며, `쿠퍼비젼코리아`로는 MFDS 업체명 조회가 0건이다.
`site/` 전체는 2026-08-28에 `쿠퍼비전`으로 정정됐다. 위 원문 줄은 당시 기록물이므로 고치지 않고 이 절로 대체한다.

### 그 밖의 근거 이동

- 제품 1 재질 `senofilcon A` — 근거를 한국 공식 제품 페이지에서 ACUVUE Technical Specification Guide로 옮겼다. 한국 페이지와 한국 IFU에는 재질명 표기가 없다.
- 제품 1 교체주기 `1일` — 근거를 한국 제품 페이지의 `1일 교체`(존재하지 않는 문자열)에서 한국 IFU의 `착용한 렌즈는 1회(1일) 착용 후 교체하여야 한다.`로 옮겼다.
- 제품 2 교체주기 `1일` — 한국 페이지에 `1일 교체` 문자열이 없고 Alcon 미국 전문가 페이지에는 교체주기 행 자체가 없다. 한국 페이지의 `1회용 렌즈`·`매일착용소프트콘택트렌즈` 표기와 MFDS 소분류를 근거로 병기했다.
- 제품 3 교체주기 `30일` — 근거를 한국 제품 페이지(`교체 주기 / 매월`)에서 쿠퍼비전코리아 2023 사양서의 `30 days replacement`로 옮기고, 한국 페이지 표기를 두 번째 출처로 병기했다.

표시값(`1일`, `1일`, `30일`)은 넷 다 그대로다. 바뀐 것은 그 값을 뒷받침하는 출처와 원문 인용이다.
