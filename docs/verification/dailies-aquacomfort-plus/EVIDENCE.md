...
1. **한국 표기 수치 일체와 한국 공식 제품 페이지 자체.** 알콘 코리아 사이트맵 66건에 이 제품이 없다(S2-1).
   ...
   **이 제품은 한국 공식 자료에서 얻을 수 있는 값이 하나도 없다.**
   2026-08-28 시점 `site/assets/data/products.js`의 9제품은 모두 한국 공식 출처를 최소 한 건 이상 갖고 있으므로,
   이 제품이 편입되면 **한국 공식 출처가 하나도 없는 첫 사례**가 된다.
