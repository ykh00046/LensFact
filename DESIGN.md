...
- Hero: the decorative 1280 × 720 WebP fills a 16:9 desktop media layer. `CONTACT LENS · EVIDENCE DESK`, one Korean evidence-first h1, the existing `[data-product-count]` statement, and `.home-evidence-ledger[data-evidence-summary]` stay in its empty left field without covering the evidence objects on the right. The image uses empty alternative text and explicit intrinsic dimensions; the ledger exposes all six computed values with truthful no-JS fallbacks.
